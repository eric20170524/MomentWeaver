...
- 可选调用项目内置 `lib/smart-asset-kit` 发布包生成背景音乐；配音/旁白 TTS 统一使用 `/Users/lm/pyProj/local-chinese-tts`。
...
The current SAK CLI supports `sak gen-audio`. MomentWeaver uses it for BGM only. Formal voiceover/TTS uses the separate local pack at `/Users/lm/pyProj/local-chinese-tts`.
...
- Use `/Users/lm/pyProj/local-chinese-tts` for course, explainer, and short-video narration.
- Start the loopback-only service with `/Users/lm/pyProj/local-chinese-tts/start_local_tts.sh`, then call `http://127.0.0.1:8765/v1/audio/speech`.
- Use the fully offline `K01` light voice by default. Use `K02`–`K04` for alternative lightweight local speakers, and use the heavier Q-series only when higher quality or expressiveness justifies its memory and latency cost.
- Use online `F*` or `M*` voices only when the user explicitly requests an online voice.
- Generated manifests should make the backend explicit, for example `provider=local-chinese-tts`, `tts_backend=local-chinese-tts`, and `voice=K01`.
- Narration must not fall back to macOS `say`. If the local service or requested local model is unavailable, fail the job instead of silently producing release audio with another backend.

OpenAI Speech-compatible request example:
...
现在 MomentWeaver 实际用到的生成与渲染能力有三类：

| 能力 | 当前用途 | 配置位置 |
|---|---|---|
| 文本大模型 | 把朋友圈文案转成微视分镜、标题、字幕、发布文案、初始音乐方向 | MomentWeaver `.env` / 设置界面 |
| BGM 音频生成 | MomentWeaver 内置用于下载/生成背景音乐 | `smart-asset-kit` 自己的 `.sak_config.json` |
| 本机中文 TTS | 为课程、解说和短视频生成分段配音/旁白 | `/Users/lm/pyProj/local-chinese-tts` |
...
- MomentWeaver 当前没有内置旁白录制/配音界面；旁白由外部课程/视频管线调用本机 `local-chinese-tts` 完成。
...
MomentWeaver 当前调用 `sak gen-audio` 生成 BGM。按所选 SAK 供应商配置对应 API key 和 audio model，例如：
...
默认使用完全离线的 `K01` 轻量音色。需要不同的轻量本地音色时选择 `K02`–`K04`；只有明确要求更高质量或更强表现力时才使用 Q 系列。在线 `F*`/`M*` 音色必须由用户明确指定。

正式旁白的音频 manifest 应标明 `provider=local-chinese-tts`、`tts_backend=local-chinese-tts`、具体 `voice` 和真实音频时长。不得回退到 macOS `say`；本机服务或所需模型不可用时应直接失败。

一句话：**MomentWeaver 配一个文本模型；BGM 配 SAK；配音/旁白 TTS 使用 `/Users/lm/pyProj/local-chinese-tts`。**
